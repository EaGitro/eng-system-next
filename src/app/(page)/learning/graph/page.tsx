...
export default async function Graph() {
    ...
    // ])
    console.log("===========================================")
    console.log("vocab", vocab, "userSynset", userSynset, "userRelation", userRelation)
    console.log("===========================================")

    // words (wordid => word)
    const words: WnjpId2Words = await fetch(process.env.NEXT_PUBLIC_WN_BACKEND_URL + "/wordids2words", {
        method: "POST",
        headers: {
    ...
    const jpnSynos: WnjpId2JpnSynos = await fetch(process.env.NEXT_PUBLIC_WN_BACKEND_URL + "/synsetids2synos", {
        method: "POST",
        headers: {
            'Content-Type': 'application/json',
        },
        body: JSON.stringify(
            { "synsetids": userSynset.map((x) => x.synsetId) }
        )
    }).then((x) => x.json())
    ...
    return (
        <>
            ...
            <CytoscapeGraph vocabs={vocab} userSynsets={userSynset} relations={userRelation} words={words} jpnSynos={jpnSynos} />
        </>
    )
